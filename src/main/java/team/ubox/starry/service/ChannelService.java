...
public class ChannelService {
    ...
    public ChannelDTO.Response open() {
        ...
        User user = userRepository.findByUsername(userDetail.getUsername()).orElseThrow(() -> new CustomException(CustomError.NOT_FOUND_USER));
    ...
    public ResponseStreamKeyDTO generateStreamKey() {
        ...
        Channel channel = channelRepository.findById(userDetail.getId()).orElseThrow(() -> new CustomException(CustomError.NOT_FOUND_CHANNEL));
        channel.updateStreamKey(key);

        ResponseStreamKeyDTO responseDto = new ResponseStreamKeyDTO();
        responseDto.setId(UUIDHelper.UUIDToString(userDetail.getId()));
    ...
    public Boolean follow(String id) {
        CustomUserDetail userDetail = AuthHelper.getAuthUser().orElseThrow(() -> new CustomException(CustomError.INVALID_TOKEN));
        Optional<Follow> follow = followRepository.findByFromUserAndToUser(userDetail.getId(), UUIDHelper.stringToUUID(id));
        if(follow.isPresent()) {
        ...
        followRepository.save(new Follow(userDetail.getId(), channel.get().getId()));
    ...
    public ResponseFollowListDTO followList() {
        CustomUserDetail userDetail = AuthHelper.getAuthUser().orElseThrow(() -> new CustomException(CustomError.INVALID_TOKEN));
        List<UUID> follows = followRepository.findAllByFromUser(userDetail.getId()).stream().map(Follow::getToUser).toList();
    ...
    public ChannelDTO.ResponseRelation relation(String channelId) {
        CustomUserDetail userDetail = AuthHelper.getAuthUser().orElseThrow(() -> new CustomException(CustomError.INVALID_TOKEN));
        Optional<Follow> follow = followRepository.findByFromUserAndToUser(userDetail.getId(), UUIDHelper.stringToUUID(channelId));
